<?php

namespace App\Constants;

class LookupMessages
{
    public const IDENTIFICATION_TYPES = 'Tipos de identificación obtenidos correctamente';

    public const COUNTRIES = 'Países obtenidos correctamente';

    public const STATES = 'Departamentos obtenidos correctamente';

    public const CITIES = 'Ciudades obtenidas correctamente';

    public const ROLES = 'Roles obtenidos correctamente';

    public const PLANS = 'Planes obtenidos correctamente';

    public const SUSCRIPTION_TYPES = 'Tipos de membresía obtenidos correctamente';

    public const LINK_TYPES = 'Tipos de link obtenidos correctamente';

    public const CONTACT_STATUSES = 'Estados de contacto obtenidos correctamente';
}
